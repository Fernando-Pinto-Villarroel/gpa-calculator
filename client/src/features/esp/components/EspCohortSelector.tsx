...
import { espCohorts } from "@/features/gpa/data/esp";
import { cn } from "@/core/lib/utils/cn";
import { useTranslations } from "next-intl";
import { useEspGpaStore } from "@/features/gpa/store/useEspGpaStore";

export function EspCohortSelector() {
  const t = useTranslations("config");
  const { selectedCohortId, setSelectedCohortId } = useEspGpaStore();
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);

  const selectedCohort =
    espCohorts.find((c) => c.id === selectedCohortId) ??
    espCohorts[espCohorts.length - 1];
  const selectedCohortNumber =
    espCohorts.findIndex((c) => c.id === selectedCohort.id) + 1;
  ...
  return (
    <div ref={ref} data-tour="esp-cohort-selector" className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className={cn(
